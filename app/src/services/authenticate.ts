...
/**
 * Connect a user to a server using the given launcher
 * 
 * @param account user account with username and password
 * @param server server on which to connect with ip and port
 * @param launcher launcher to use to connect to the server
 */
function connect(account: userAccount, server: Server, launcher : Launcher): void {
    ...
    window.host?.execute(connectCommand);
}
